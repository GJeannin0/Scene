...
namespace gl {
	...
	class HelloNormal : public Program
	{
	public:
		void Init() override;
	...
	protected:
		...
		std::unique_ptr<Texture> texture_normal_ = nullptr;
	...
	void HelloNormal::IsError(const std::string& file, int line) const
	{
	...
	void HelloNormal::Init()
	{
		...
		texture_diffuse_ = std::make_unique<Texture>(
			path + "data/textures/brickwall.jpg",1);
		texture_specular_ = std::make_unique<Texture>(
			path + "data/textures/brickwall.jpg",2);
		texture_normal_ = std::make_unique<Texture>(
			path + "data/textures/brickwall_normal.jpg",3);
		
		
		shaders_ = std::make_unique<Shader>(
			path + "data/shaders/hello_normal/normal.vert",
			path + "data/shaders/hello_normal/normal.frag");
		...
		texture_normal_->Bind(2);
		shaders_->SetInt("NormalMap", 2);
	...
	void HelloNormal::SetModelMatrix(seconds dt)
	{
		model_ = glm::rotate(glm::mat4(1.0f), time_/22, glm::vec3(0.f, 1.f, 0.f));
		inv_model_ = glm::transpose(glm::inverse(model_));
	}

	void HelloNormal::SetViewMatrix(seconds dt)
	{
		view_ = camera_->GetViewMatrix();
	}

	void HelloNormal::SetProjectionMatrix()
	{
	...
	void HelloNormal::SetUniformMatrix() const
	{
		shaders_->Use();
	...
	void HelloNormal::Update(seconds dt)
	{
